...
- **Vai trò cho phép:** `CUSTOMER` (được phân quyền qua role token)
- **Request Body:**
  ```json
  {
    "bookingId": 25,
...
#### 1. Quản lý Đặt phòng (Admin Booking Search)
...
- **Response (200 OK):** Trạng thái booking sau khi Admin xử lý thủ công (phục vụ khách đặt tại quầy).
