...
| **Staff** | Nhân viên vận hành chung (vai trò cha/cũ). |
| **Receptionist** | Nhân viên lễ tân. Có quyền xem danh sách phòng, cập nhật trạng thái phòng (Clean/Dirty), tạo/cập nhật và xử lý duyệt/hủy đặt phòng thủ công của admin. |
| **Housekeeper** | Nhân viên dọn phòng. Có quyền xem danh sách phòng và cập nhật trạng thái dọn dẹp (Clean/Dirty) của phòng. |
| **Admin** | Quản trị viên hệ thống. Có toàn quyền quản lý Inventory, cấu hình tham số, quản lý tài khoản, kiểm duyệt đánh giá, đặt/hủy đặt phòng. |
| **Director** | Ban giám đốc. Có quyền xem báo cáo doanh thu chiến lược, hiệu suất sử dụng phòng toàn hệ thống và xuất báo cáo Excel. |
| **System** | Hệ thống tự động xử lý ngầm (cron-jobs, event listeners, payment webhooks). |

### Bảng ma trận phân quyền (RBAC Matrix)

| API Path Prefix | Guest | Customer | Receptionist | Housekeeper | Admin | Director | System |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| `/api/v1/auth/**` | ✔ | ✔ | ✔ | ✔ | ✔ | ✔ | ✔ |
...
| `/api/v1/admin/bookings` (GET/POST/PUT) | - | - | ✔ | - | ✔ | - | - |
| `/api/v1/admin/bookings/{id}/status` | - | - | ✔ | - | ✔ | - | - |
| `/api/v1/admin/bookings/**` (DELETE) | - | - | - | - | ✔ | - | - |
| `/api/v1/admin/users/**` | - | - | - | - | ✔ | - | - |
| `/api/v1/reports/reviews` (GET) | - | - | - | - | ✔ | - | - |
| `/api/v1/reports/reviews/{id}/moderate` | - | - | - | - | ✔ | - | - |
| `/api/v1/reports/**` (Báo cáo khác) | - | - | - | - | ✔ | ✔ | - |
| `/api/v1/payments/create` | - | ✔ | ✔ | - | ✔ | - | - |
| `/api/v1/payments/webhook` | ✔ | - | - | - | - | - | ✔ |
| `/api/v1/payments/{bookingId}/refund` | - | - | - | - | ✔ | - | - |
| `/api/v1/reviews` (POST) | - | ✔ | - | - | - | - | - |
| `/api/v1/hotels/{hotelId}/reviews` (GET) | ✔ | ✔ | ✔ | ✔ | ✔ | ✔ | - |
...
#### UC-36: Cập nhật trạng thái buồng phòng (Update Room Status)
- **Actor:** Housekeeper / Receptionist / Admin
- **Description:** Cập nhật trạng thái làm sạch hoặc buồng phòng (AVAILABLE - Sạch sẽ sẵn sàng đón khách, UNAVAILABLE - Bẩn hoặc đang sửa chữa cần dọn dẹp).
- **Business Rules:**
  - Nhân viên dọn phòng (Housekeeper) và lễ tân (Receptionist) đều có quyền thay đổi trạng thái buồng phòng.
  - Khi phòng được chuyển sang trạng thái UNAVAILABLE, nó sẽ không hiển thị trên danh sách phòng trống khả dụng để khách đặt.
- **REST API Endpoints:**
  - `PUT /api/v1/rooms/{id}/availability?available={boolean}`
  - `GET /api/v1/rooms/hotel/{hotelId}` (Lấy danh sách toàn bộ phòng thuộc khách sạn phục vụ công tác buồng phòng/quầy lễ tân).
...
#### UC-31: Đăng, Xem & Kiểm duyệt đánh giá (Reviews System)
- **Actors:** Customer (Đăng đánh giá), Guest/Customer (Xem đánh giá), Admin (Kiểm duyệt)
- **Business Rules:**
  - Khách hàng chỉ có thể gửi đánh giá cho những booking ở trạng thái `COMPLETED` và thuộc sở hữu của chính họ.
  - Mỗi booking (`booking_id`) chỉ được phép tạo tối đa **1 đánh giá**.
  - Khi một đánh giá được tạo mới (hoặc bị ẩn bởi Admin), điểm đánh giá trung bình (`rating`) của khách sạn liên quan sẽ tự động được cập nhật lại dựa trên các đánh giá có trạng thái `VISIBLE`.
- **REST API Endpoints:**
  - **Đăng đánh giá (Customer):** `POST /api/v1/reviews`
    ...
    - **Response Body (ReviewResponse):**
      ...
      ```
  - **Xem danh sách đánh giá của khách sạn (Public):** `GET /api/v1/hotels/{hotelId}/reviews?page=0&size=10`
  - **Xem danh sách để kiểm duyệt (Admin):** `GET /api/v1/reports/reviews?status=ALL&page=0&size=20`
  - **Thực hiện kiểm duyệt (Admin):** `PATCH /api/v1/reports/reviews/{id}/moderate`
