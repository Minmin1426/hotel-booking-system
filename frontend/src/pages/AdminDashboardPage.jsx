...
export default function AdminDashboardPage() {
  ...
  const handleApproveRefund = async (bookingId, bookingCode) => {
    const confirmed = window.confirm(`Bạn có chắc chắn muốn PHÊ DUYỆT hoàn tiền cho Booking ${bookingCode}? Tiền hoàn sẽ được hoàn lại tự động qua cổng thanh toán theo chính sách.`);
    if (!confirmed) return;
    ...
    try {
      await PaymentService.processRefund(bookingId);
      alert("Đã phê duyệt hoàn tiền thành công!");
      loadBookings();
    } catch (err) {
      setError("Phê duyệt hoàn tiền thất bại: " + err.message);
    } finally {
      setActionLoadingId(null);
    }
  };
                          ...
                          <tr key={booking.bookingId} className="hover:bg-[#fafafc] transition-colors">
                            ...
                            <td className="p-4 text-right">
                              <div className="flex gap-1.5 justify-end items-center">
                                ...
                                {booking.status === 'CANCELLED' && booking.paymentStatus === 'REFUND_PENDING' && (
                                  <button
                                    onClick={() => handleApproveRefund(booking.bookingId, booking.bookingCode)}
                                    disabled={actionLoadingId === booking.bookingId}
                                    className="px-2.5 py-1.5 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-700 hover:bg-emerald-200 active:scale-95 transition-all cursor-pointer border border-emerald-300"
                                  >
                                    {actionLoadingId === booking.bookingId ? '...' : '✓ Approve Refund'}
                                  </button>
                                )}
                              </div>
                            </td>
