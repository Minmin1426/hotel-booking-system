...
export default function ProfilePage() {
  ...
  const handleShowQr = async (ticket) => {
    ...
    try {
      const token = sessionStorage.getItem("accessToken");
      const baseApiUrl = import.meta.env.VITE_API_URL || (window.location.origin.includes("localhost") ? "http://localhost:8080/api/v1" : "https://hotel-booking-system-0wv2.onrender.com/api/v1");
      const response = await fetch(`${baseApiUrl}/users/me/meal-tickets/${ticket.ticketId}/qr`, {
        headers: {
  ...
  const handleDownloadInvoice = async (bookingId, bookingCode) => {
    try {
      ...
      const paymentId = paymentData.paymentId || paymentData.data?.paymentId;
      if (!paymentId) {
              ...
              {vouchersLoading && vouchers.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {vouchers.map((v) => {
                    ...
                    const isInvalid = isExpired || isFullyUsed || isUsed;
