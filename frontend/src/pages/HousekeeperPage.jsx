...
export default function HousekeeperPage() {
  ...
  const handleToggleStatus = async (roomId, currentStatus) => {
    ...
    try {
      ...
      setError(err.message || "Failed to update room cleaning status.");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f7] flex flex-col font-sans">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-6 py-10">
        {/* Banner Card */}
        <div className="bg-gradient-to-r from-cyan-600 to-indigo-600 rounded-3xl p-8 md:p-12 text-white shadow-xl mb-8 text-left">
          <span className="px-3 py-1 rounded-full bg-white/10 text-xs font-bold uppercase tracking-wider">Housekeeping Console</span>
          <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight mt-3">Room Cleaning & Status Report</h1>
        ...
        <div className="bg-white rounded-2xl p-6 border border-[#e8e8ed] shadow-sm mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex flex-1 flex-col md:flex-row items-stretch md:items-center justify-end gap-3">
            ...
            <div className="flex flex-col text-left">
              <label className="text-[10px] font-bold text-[#86868b] uppercase tracking-wider mb-1.5">Cleaning Status</label>
              <select
                value={statusFilter}
              ...
              >
                ...
                <option value="UNAVAILABLE">Dirty / Needs Clean</option>
              </select>
            </div>
          </div>
        </div>
                ...
                <div>
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full border ${getStatusBadgeClass(room.status)}`}>
                      {room.status === 'AVAILABLE' ? 'Clean' : 'Dirty / Cleaning'}
                    </span>
                  </div>
